...
import {ChipTraits, ChipVersion, NftCardTraits, Node, NodeTraits} from "./libraries/DataTypes.sol";
import {BatchSizeZero, CallerNotStaking} from "./libraries/Errors.sol";
...
contract Chips is IChips, Initializable, ERC721 {
    ...
    /// @inheritdoc IChips
    function initialize(string calldata name_, string calldata symbol_, address staking_)
        external
        override
        reinitializer(2)
    {
    ...
    function nodeImageAndAttributesURI(address nodeAddr)
        external
        view
        override
        returns (string memory)
    {
        ...
        (string memory imageSVG, string memory attributes) =
            SVGGeneratorV2.generateSVGAndAttributes(nodeTraits, chipTraits);
    ...
    function _generateChipImage(uint256 tokenId)
        internal
        view
        returns (NodeTraits memory, ChipTraits memory, NftCardTraits memory)
    {
    ...
    function _getNodeTraits(address nodeAddr, ChipVersion chipVersion)
        internal
        view
        returns (NodeTraits memory)
    {
        Node memory node = IStaking(_staking).getNode(nodeAddr);
        ...
        NodeTraits memory nodeTraits = NodeTraits({
            frameId: _calTraitId(
                nodeTraitId,
                frameCount,
                uint256(colorCount) * uint256(chipDetailCount) * uint256(colorCount)
            ...
            chipDetailColor: _calTraitId(
                nodeTraitId, colorCount, uint256(chipDetailCount) * uint256(colorCount)
            ),
    ...
    function _getOtherTraits(address nodeAddr, uint256 tokenId)
        internal
        view
        returns (ChipTraits memory, uint8)
    {
    ...
    function _getOtherTraitsBySeed(uint256 seed, ChipVersion chipVersion)
        internal
        pure
        returns (ChipTraits memory, uint8 nftCardId)
    {
        uint8 eyeCount;
        uint8 mouthCount;
        ...
        if (chipVersion == ChipVersion.V1) {
            (eyeCount, mouthCount, headShapeCount, headDetailCount) =
                SVGGenerator.getChipTraitsCount();
            (colorCount,,,) = SVGGenerator.getNodeTraitsCount();
            nftCardCount = 1; // V1 doesn't have nftCardCount, set to 1
        } else {
            (eyeCount, mouthCount, headShapeCount, headDetailCount, nftCardCount) =
                SVGGeneratorV2.getChipTraitsCount();

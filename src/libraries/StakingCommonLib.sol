...
library StakingCommonLib {
    ...
    function increaseStakingPool(DataTypes.Node storage node, uint256 amount) internal {
    ...
    function decreaseStakingPool(DataTypes.Node storage node, uint256 amount) internal {
        node.stakingPoolTokens -= amount;
        uint256 newTotalStakingPoolTokens = StorageLib.getTotalStakingPoolTokens() - amount;
        StorageLib.setTotalStakingPoolTokens(newTotalStakingPoolTokens);
    }
    ...
    function increaseOperationPool(DataTypes.Node storage node, uint256 amount) internal {
    ...
    function decreaseOperationPool(DataTypes.Node storage node, uint256 amount) internal {
        node.operationPoolTokens -= amount;
    ...
    function increaseSlashingPoolByRecord(DataTypes.SlashRecord storage record) internal {
    ...
    function decreaseSlashingPoolByRecord(DataTypes.SlashRecord storage record) internal {
        uint256 newSlashingPoolTokens = StorageLib.getTotalSlashingPoolTokens() - _totalSlashedAmount(record);
        StorageLib.setTotalSlashingPoolTokens(newSlashingPoolTokens);
    }

    function _totalSlashedAmount(DataTypes.SlashRecord storage record) internal view returns (uint256) {
